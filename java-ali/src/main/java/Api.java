...
public class Api {
    ...
    public static String getMark(String markdemo) {
        //System.out.println(markdemo);
        //输出实例
        String charsen = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
        //随机字母
        String charsall = "/01234567890+ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
        //随机字母数字符号
        String alimark = markdemo.substring(0,21)+String.valueOf(charsen.charAt((int) (Math.random() * 52)));
        //实例前21位+一个随机字母
        int iw = 0;
        while(iw < 85){
            char c = charsall.charAt((int) (Math.random() * 64));
            alimark = alimark + c;
            iw = iw+1;
        }
        //生成85位随机字母数字符号
    ...
    public static void checkUserConfig() {
        try {
            ...
            headers.put("refer", "https://2021001157662937.hybrid.alipay-eco.com/2021001157662937/0.2.2204291231.54/index.html#pages/addressPackage/address/address?__appxPageId=103&from=mine");
    ...
    public static void allCheck() {
        try {
            ...
            Map<String, String> headers = UserConfig.getHeaders();
            headers.put("refer","https://2021001157662937.hybrid.alipay-eco.com/2021001157662937/0.2.2204291231.54/index.html#pages/mainPackage/cart/cart");
            httpRequest.addHeaders(headers);
    ...
    public static Map<String, Object> getMultiReserveTime(String addressId, Map<String, Object> cartMap) {
        try {
            ...
            Map<String, String> headers = UserConfig.getHeaders();
            headers.put("refer","https://2021001157662937.hybrid.alipay-eco.com/2021001157662937/0.2.2204291231.54/index.html#pages/orderPackage/makeOrder/newMakeOrder?__appxPageId=105&data=MAKE_ORDER_DATA&hasOnion=0");
            httpRequest.addHeaders(headers);
    ...
    public static boolean addNewOrder(String addressId, Map<String, Object> cartMap, Map<String, Object> multiReserveTimeMap, Map<String, Object> checkOrderMap) {
        try {
            ...
            Map<String, String> headers = UserConfig.getHeaders();
            headers.put("refer","https://2021001157662937.hybrid.alipay-eco.com/2021001157662937/0.2.2204291231.54/index.html#pages/orderPackage/makeOrder/newMakeOrder?__appxPageId=103&data=MAKE_ORDER_DATA&hasOnion=0");
            httpRequest.addHeaders(headers);
            Map<String, Object> request = UserConfig.getBody(headers);
            request.put("showMsg", "false");
            request.put("showData", "true");
            ...
            paymentOrderMap.put("pay_type", 25);
            ...
            context.put("end", new HashMap<>());
            for (int i = 0; i < 20; i++) {
                System.out.println("恭喜！已成功下单 当前下单总金额：" + cartMap.get("total_money"));
            }
            return true;
        } catch (Exception e) {
            e.printStackTrace();
